...
public interface StuTaskMapper extends Mapper<StuTask> {
    ...
    @Select("select * from stu_task where task_id = #{taskId} order by submit_time desc limit #{limit} offset #{offset}")
    List<StuTask> getStuTaskList(@Param("taskId") Integer taskId, @Param("limit") Integer limit, @Param("offset") Integer offset);

    @Select("select count(distinct(stu_id)) from stu_task where task_id = #{taskId}")
